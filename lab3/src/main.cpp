...
void createGrid(MPI_Comm& gridComm, MPI_Comm& rowComm, MPI_Comm& colComm, int& rank, int& size, int& p1, int& p2, int& x, int& y) {
...
void scatterA(const std::vector<double>& A, std::vector<double>& ARow, int N, int localRows, MPI_Comm gridComm, int x, int y) {
    ...
    if (x == 0) {
        ...
        MPI_Scatter(sendBuf, localRows * N, MPI_DOUBLE, ARow.data(), localRows * N, MPI_DOUBLE, 0, col0Comm);

        MPI_Comm_free(&col0Comm);
    }
}

void scatterB(const std::vector<double>& B, std::vector<double>& BCol, int N, int localCols, MPI_Comm gridComm, int x, int y) {
    ...
    if (y == 0) {
        ...
        MPI_Scatter(sendBuf, 1, resizedStripeType, BCol.data(), N * localCols, MPI_DOUBLE, 0, row0Comm);
...
void broadcastStripes(std::vector<double>& Arow, std::vector<double>& Bcol, int localRows, int localCols, int N, MPI_Comm rowComm, MPI_Comm colComm) {
    MPI_Bcast(Arow.data(), localRows * N, MPI_DOUBLE, 0, rowComm);
    MPI_Bcast(Bcol.data(), N * localCols, MPI_DOUBLE, 0, colComm);
}

void multiplyLocal(const std::vector<double>& Arow, const std::vector<double>& Bcol, std::vector<double>& Cloc, int localRows, int localCols, int N) {
...
void gatherC(std::vector<double>& C, const std::vector<double>& Cloc, int localRows, int localCols, int N, int rank, int p1, int p2, MPI_Comm gridComm) {
